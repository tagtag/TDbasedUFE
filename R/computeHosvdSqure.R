#' Title
#' Compute higher order singular value decomposition from the tensor generated from squared matrix
#' @param Z A tensor including sample names, feature values, associated with featureRange and sample properties
...
computeHosvdSqure <-function(Z,dims=unlist(lapply(dim(Z@value),function(x){min(10,x)})),
                             scale=T)
{
    if (scale)
    {
        mean <- apply(Z@value,length(dim(Z@value)),mean)
        Z@value <- aperm(aperm(Z@value,c(length(dim(Z@value)),c(1:(length(dim(Z@value))-1))))/mean,
