...
(person). If the measurements are performed over mutliple conditions, e.g.,
...
various way to compose tensor. Usueally, tensor decomposition (TD) is to
decompose tensor into pricuct some of vectors and tensors. 
...
![Tensor Decompositions](./fig1.jpg)

which can be mathematically expressed as
$$
x_{ijk} = \sum_\ell \lambda_\ell u_{\ell i} u_{\ell j} u_{\ell k}
$$
Alternatively, more asvanced way, tensor train decomposition,  
can decomposes the tensor to product of a mixuture of matrix and tensor as
$$
...
are singular value matrices and are orthogonal matricies. 

Although there are no uniqueness of Tucker decomposition, we sepfifically
employed 
higher order singular value decomposition (HOSVD) [@Taguchi2020] to get
Tucker decomposition.
The reason why we employed Tucker decomosition was fully described in 
my book[@Taguchi2020].
...
whose expression is distict between patients and healthy controls only at one 
...
correspond to ditinction between patient and healthy control and tissue 
specificity, respectvely (see (3)). Finally, with assuming that 
...
FE proposed ten years ago, we recently recognize that the oprimization of 
...
perfoemance is as follows. Previously, we computed SD of 
...
of $i$th does not follow Gaussian distrbution and has too large 
absolute values.
...
with the opitimized $\sigma_{\ell_1}$.  ](./fig3.jpg)
...
fully generated from Gaussian. The smallest $\sigma_h$ correctly corresonds 
...
$$
where $\ell_1$ and $\ell_2$ are exchangable since $x_{jj'k} = x_{j'jk}$.
Singular values attributed to features can be retrived as
$$
